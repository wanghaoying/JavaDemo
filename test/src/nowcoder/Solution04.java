...
public class Solution04 {
     ...
     *      由于将数组进行整体的反转可能会发生溢出，我们的目的是判断是否是回文数，那么其实
     *   只需要反转一半的长度就够了，比较前半部分，与后半部分反转后的数字是否相等
     *
     * 2、计算出这个数字整个的位数digit，然后可以通过x/(10^(digit-1))来获取最高位上的数字，
     *  以及通过x%10获取最低位上的数字，然后依次对各个数位进行比较
     */
    public boolean isPalindrome (int x) {
        // 如果一个数字是负数 或者 是正数而且还是10的倍数，那么这个数字一定不是回文数
        if (x < 0 || (x != 0 && x %10 == 0)){
            return false;
        }
        // 获取数字的前半部分 和 后半部分反转后的结果
        int reverse = 0;
        while (x > reverse){
            reverse = reverse*10 + x%10;
            x /= 10;
        }
        // reverse/10 == x 主要是为了解决x的初始位数为奇数时的情况，这样在生成
        // reverse的时候，会把中间位加到reverse上，所以要除以10
